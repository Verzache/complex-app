...
Follow.prototype.validate = async function(action){
    // followerd username must exist in database
    let followedAccount = await usersCollection.findOne({username: this.followedUsername}) // finds for username in database collection named users
    if(followedAccount){
    ...
    let doesFollowAlreadyExist = await followsCollection.findOne({followedId: this.followedId,authordId: new ObjectID(this.authorId)})
    if(action == "create"){
        if(doesFollowAlreadyExist){this.errors.push("You are already following this user")}
    }
    if(action == "delete"){
        if(!doesFollowAlreadyExist){this.errors.push("You cannot stop following somone you already follow")}
    }
    ...
    return new Promise(async(resolve,reject)=>{
        try{
            ...
            followers = followers.map(function(follower){
                //create a user
                let user = new User(follower,true)//true will auto pull gravitar
    ...
    return new Promise(async(resolve,reject)=>{
        try{
            ...
            following = following.map(function(following){
                //create a user
                let user = new User(following,true)//true will auto pull gravitar
