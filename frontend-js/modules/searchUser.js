// for user search
// extra feature for next version
...
export default class SearchUser{
    ...
    constructor(){
      this._csrf = document.querySelector('[name="_csrf"]').value
        this.injectHTML()
        this.headerUserSearchIcon = document.querySelector(".header-user-icon")
    ...
    sendRequest(){
      axios.post('/usersearch',{ _csrf: this._csrf, searchTerm: this.inputField.value}).then(response => {
    ...
    renderResultsHTML(posts){
      if(posts.length){
        // DOMpurify prevents (malicious)js from running in front END
        ...
        this.resultsArea.innerHTML = `<p class="alert alert-danger text-center shadow-sm">Sorry we coudn't find any matching results.</p>`        
      }
      this.hideLoaderIcon()
